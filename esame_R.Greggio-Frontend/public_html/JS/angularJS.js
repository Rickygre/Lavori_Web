...
app.controller('loginCtrl',
        function ($scope, $http) {
            ...
            $scope.login = function () {
                ...
                let req = {
                    url: "http://localhost:8080/esame_R.Greggio/resources/utenti/login",
                    method: "POST",
            ...
            $scope.getMyBkms = function () {
                let  myurl = "http://localhost:8080/esame_R.Greggio/resources/segnalibri/" + sessionStorage.getItem("myid");
                let req = {
                    url: myurl,
                    method: "GET",
                    headers: {
            ...
            $scope.creaBkm = function () {
                let  myurl = "http://localhost:8080/esame_R.Greggio/resources/segnalibri/";
                let sh = false;
                if ($scope.sh === "true")
                    sh = true;
                ...
                $http(req).then(
                        function (response) { //caso successo   
                            console.log(response);
                            $scope.msginsbkm = "Segnalibro inserito correttamente!!";
                            $scope.getMyBkms();

                        },
                        function (response) {
                            $scope.msginsbkm = "ATTENZIONE segnalibro non creato!! completare i campi richiesti!";
                        });
            };
            ...
            $scope.logout = function () {
                ...
                window.location.href = "/esame_R.Greggio-Frontend/index_esame.html";
            };
